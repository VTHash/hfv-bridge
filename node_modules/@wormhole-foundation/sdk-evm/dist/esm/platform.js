import { PlatformContext, Wormhole, chainToPlatform, decimals, encoding, isNative, nativeChainIds, networkPlatformConfigs, getWalletBalances, } from '@wormhole-foundation/sdk-connect';
...
export class EvmPlatform extends PlatformContext {
    ...
    getRpc(chain) {
        ...
        if (chain in this.config && this.config[chain].rpc) {
            const provider = new JsonRpcProvider(this.config[chain].rpc, nativeChainIds.networkChainToNativeChainId.get(this.network, chain), {
                staticNetwork: true,
            });
    ...
    static async getBalances(network, chain, rpc, walletAddr, indexers) {
        const balances = await getWalletBalances(walletAddr, network, chain, indexers);
