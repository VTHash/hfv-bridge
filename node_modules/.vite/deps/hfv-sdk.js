...
// node_modules/hfv-sdk/dist/index.mjs
...
var HFVPrices = class {
  ...
  async getNativePrices(chainIds) {
    const ids = (chainIds == null ? void 0 : chainIds.length) ? chainIds : getAllSupportedChains().map((c) => c.chainId);
    if (!ids.length) return {};
    const param = ids.join(",");
    const { data } = await this.http.get("/prices/native", {
      params: { chainIds: param }
    });
    return (data == null ? void 0 : data.prices) || {};
  }
  /**
...
var TOKENS_DIR = import_path.default.join(__dirname, "tokens");
function loadTokens() {
  const registry = {};
  const files = import_fs.default.readdirSync(TOKENS_DIR).filter((f) => f.endsWith(".json"));
  for (const file of files) {
    const chainName = file.replace(".json", "");
    const fullPath = import_path.default.join(TOKENS_DIR, file);
    try {
      const content = JSON.parse(import_fs.default.readFileSync(fullPath, "utf8"));
...
//# sourceMappingURL=hfv-sdk.js.map
