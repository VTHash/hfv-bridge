...
export class GatewayTransfer {
    ...
    constructor(wh, transfer, gateway, gatewayIbc) {
        ...
        this.gatewayAddress = {
            ...
            address: toNative(GatewayTransfer.chain, this.wh.config.chains[GatewayTransfer.chain].contracts.gateway),
        };
    ...
    static async _fromMsgId(wh, from, timeout) {
        ...
        if (payload) {
            try {
                ...
                const recipientAddress = encoding.bytes.decode(encoding.b64.decode(maybeWithPayload.recipient));
    ...
    async _transfer(signer) {
        ...
        const xfer = tb.transfer(this.transfer.from.address, this.gatewayAddress, tokenAddress, this.transfer.amount, encoding.bytes.encode(JSON.stringify(this.msg)));
    ...
    async fetchAttestation(timeout) {
        ...
        if (this.fromGateway()) {
            ...
            if (!this.toGateway()) {
                ...
                const whm = await retry(task, retryInterval, timeout, "Gateway:IbcBridge:LookupWormholeMessageFromIncomingIbcMessage");
                if (!whm)
            ...
            const redeemed = await retry(isRedeemedTask, vaaRedeemedRetryInterval, timeout, "Gateway:TokenBridge:IsVaaRedeemed");
            if (!redeemed)
            ...
            const destTransfer = await retry(destTransferTask, transferCompleteInterval, timeout, "Destination:IbcBridge:WormchainTransferCompleted");
            if (!destTransfer)
