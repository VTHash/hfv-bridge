...
class Wormhole {
    ...
    async tokenTransfer(token, amount, from, to, protocol, options) {
        switch (protocol) {
            case "TokenBridge":
    ...
    async getExecutorCapabilities() {
        return await (0, sdk_definitions_2.fetchCapabilities)(this.config.executorAPI);
    }
    async getExecutorQuote(srcChain, dstChain, relayInstructions) {
        return await (0, sdk_definitions_2.fetchQuote)(this.config.executorAPI, srcChain, dstChain, relayInstructions);
    }
    async getExecutorTxStatus(txHash, chain) {
        return await (0, sdk_definitions_2.fetchStatus)(this.config.executorAPI, txHash, chain);
    }
    /**
