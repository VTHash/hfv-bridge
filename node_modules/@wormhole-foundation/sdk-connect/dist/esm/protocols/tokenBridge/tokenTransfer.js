import { amount, encoding, finality, guardians, time, toChain as toChainName, toChainId, deserializeLayout, serializeLayout, } from "@wormhole-foundation/sdk-base";
import { TokenBridge, UniversalAddress, canonicalAddress, deserialize, isNative, isSameToken, isTokenId, isTokenTransferDetails, isTransactionIdentifier, isWormholeMessageId, serialize, toNative, toUniversal, relayInstructionsLayout, signedQuoteLayout, nativeTokenId, } from "@wormhole-foundation/sdk-definitions";
import { signSendWait } from "../../common.js";
import { DEFAULT_TASK_TIMEOUT } from "../../config.js";
import { chainToPlatform } from "@wormhole-foundation/sdk-base";
import { TransferState, isAttested, isInReview, isRedeemed, isRelayFailed, isSourceFinalized, isSourceInitiated, } from "../../types.js";
...
(function (TokenTransfer) {
    ...
    async function* track(wh, receipt, timeout = DEFAULT_TASK_TIMEOUT, fromChain, toChain) {
        ...
        if (isAttested(receipt) &&
            ...
            if (relayStatus === RelayStatus.Failed ||
    ...
    async function lookupDestinationToken(srcChain, dstChain, token) {
        ...
        if (lookup.chain === dstChain.chain) {
            ...
            if (canonicalAddress({ chain: dstChain.chain, address: destWrappedNative }) ===
            ...
            return {
                ...
                destinationToken: {
                    ...
                    amount: referrerFee?.remainingAmount
                        ? amount.units(amount.scale(amount.fromBaseUnits(referrerFee.remainingAmount, srcDecimals), dstDecimals))
    ...
    async function getExecutorQuote(wh, srcChain, dstChain, gasLimit, msgValue, nativeGas) {
        ...
        const executorQuote = await wh.getExecutorQuote(srcChain.chain, dstChain.chain, encoding.hex.encode(relayInstructionsBytes, true));
        if (!executorQuote.estimatedCost) {
            throw new Error("No estimated cost");
        }
        const estimatedCost = BigInt(executorQuote.estimatedCost);
        const signedQuote = deserializeLayout(signedQuoteLayout, encoding.hex.decode(executorQuote.signedQuote));
        return {
            signedQuote,
            estimatedCost,
            relayInstructions,
