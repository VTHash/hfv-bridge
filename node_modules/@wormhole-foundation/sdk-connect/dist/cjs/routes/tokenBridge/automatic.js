...
class AutomaticTokenBridgeRoute extends route_js_1.AutomaticRoute {
    ...
    async validate(request, params) {
        try {
            ...
            if ((0, sdk_base_1.chainToPlatform)(request.fromChain.chain) === "Solana") {
                const isToken2022 = await request.fromChain.isToken2022(request.source.id.address);
                if (isToken2022) {
                    return {
                        valid: false,
                        params,
                        error: new Error("Transfers of Token-2022 assets from SVM chains are not supported via the Automatic Token Bridge route. Please use the Executor Token Bridge route instead."),
                    };
                }
            }
            if ((0, sdk_base_1.chainToPlatform)(request.toChain.chain) === "Solana") {
                const isToken2022 = await request.toChain.isToken2022(request.destination.id.address);
                if (isToken2022) {
                    return {
                        valid: false,
                        params,
                        error: new Error("Transfers of Token-2022 assets to SVM chains are not supported via the Automatic Token Bridge route. Please use the Executor Token Bridge route instead."),
                    };
                }
            }
