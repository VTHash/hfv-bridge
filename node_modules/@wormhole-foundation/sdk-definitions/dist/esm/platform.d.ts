...
export interface PlatformUtils<P extends Platform> {
    ...
    getDecimals<C extends PlatformToChains<P>>(network: Network, chain: C, rpc: RpcConnection<P>, token: TokenAddress<C>): Promise<number>;
    /** Get the balance of a token for a given wallet address */
    getBalance<C extends PlatformToChains<P>>(network: Network, chain: C, rpc: RpcConnection<P>, walletAddr: string, token: TokenAddress<C>): Promise<bigint | null>;
...
export interface PlatformIndexerUtils<N extends Network, P extends Platform> {
    ...
    getBalances(network: Network, chain: PlatformToChains<P>, rpc: RpcConnection<P>, walletAddress: string, indexer?: IndexerConnection<P>): Promise<Balances>;
}
export declare function supportsIndexerUtils(thing: any): thing is PlatformIndexerUtils<Network, Platform>;
...
export declare abstract class PlatformContext<N extends Network, P extends Platform> {
    ...
    getProtocol<PN extends ProtocolName>(protocol: PN, rpc: RpcConnection<P>): Promise<ProtocolInstance<P, PN, N>>;
    /** Get the underlying ProtocolInitializer to construct yourself */
    getProtocolInitializer<PN extends ProtocolName>(protocol: PN): ProtocolInitializer<P, PN, Network>;
